...
*/
	...
	set_menu_pad(playlistNames, 1);
			...
			case 'r':
				rewind(fptr);
				unpost_menu(likeSongs);
				int moveDown = item_index(current_item(likeSongs));
				FILE*lksongsfptr = fopen(strcat(strdup(argv1), ".plists"), "r");
				//fprintf(lksongsfptr, "");
				
				fseek(lksongsfptr, 0L, SEEK_END);
				int lksongssize = ftell(lksongsfptr);
				char*printfile = malloc(lksongssize);
				char*lkline = malloc(lksongssize);
				strcpy(printfile, "");
				rewind(lksongsfptr);
				int lnamt = 0;
				while(fgets(lkline, lksongssize, lksongsfptr)) {
					lnamt++;
					if(strcmp(lkline, stradd((char*)item_name(current_item(likeSongs)), "\n"))) {
						strcat(printfile, lkline);
					}
				}
				
				fclose(lksongsfptr);
				lksongsfptr = fopen(strcat(strdup(argv1), ".plists"), "w");
				fprintf(lksongsfptr, "%s", printfile);
				...
				refresh();
				sleep(1);https://www.google.com/search?client=firefox-b-d&q=set+menu+position+ncurses
